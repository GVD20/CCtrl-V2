...
| 28–29 | `reserved` | `uint16` | 保留，当前发送 `0` |
...
仓库中的 [`tools/protocol_v2.py`](../tools/protocol_v2.py) 按以下顺序解析字节流：

1. 定位 `0xA5`，读取前 5 字节中的小端 `payload_len`，并匹配固定值 `30`。
2. 收齐 39 字节后，比较偏移 0–3 的 CRC8 与偏移 4 的值。
3. 比较偏移 5–6 的小端命令号与 `0x0302`，并验证偏移 0–36 的 CRC16。
4. 按第 3 节的字段表解析 payload，返回带 `sequence` 的 `HostFrame` 供上层统计。
5. CRC 或格式校验失败的候选帧被丢弃，解析器从后续字节继续搜索 `0xA5`。
